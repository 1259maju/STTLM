...
from lib.data_prepare import get_dataloaders1
from models.sttlm.STTLM import STTLM
...
def train(
    ...
    for epoch in range(max_epochs):
        ...
        #if val_loss < min_val_loss and train_loss > val_loss:
...
if __name__ == "__main__":
    ...
    model_name = STTLM.__name__

    with open(f"./models/sttlm/{model_name}.yaml", "r") as f:
        cfg = yaml.safe_load(f)
    cfg = cfg[dataset]

    # -------------------------------- load model -------------------------------- #

    model = STTLM(**cfg["model_args"])
    ...
    print(f'Number of parameters in about llama: {count_parameters(model.peft_model)}')
